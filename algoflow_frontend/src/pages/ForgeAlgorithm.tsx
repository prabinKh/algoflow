...
export default function ForgeAlgorithm() {
  ...
  const [formData, setFormData] = useState<Partial<Algorithm>>({
    ...
    code: {
      python: { classCode: '', functionCode: '', recursiveCode: '', classOutcome: '', classRuntime: '', functionOutcome: '', functionRuntime: '', recursiveOutcome: '', recursiveRuntime: '' },
      cpp: { classCode: '', functionCode: '', recursiveCode: '', classOutcome: '', classRuntime: '', functionOutcome: '', functionRuntime: '', recursiveOutcome: '', recursiveRuntime: '' },
      c: { classCode: '', functionCode: '', recursiveCode: '', classOutcome: '', classRuntime: '', functionOutcome: '', functionRuntime: '', recursiveOutcome: '', recursiveRuntime: '' },
      rust: { classCode: '', functionCode: '', recursiveCode: '', classOutcome: '', classRuntime: '', functionOutcome: '', functionRuntime: '', recursiveOutcome: '', recursiveRuntime: '' },
    },
    explanation: {
      problem: '',
      intuition: '',
  ...
  const handleActivateAssistant = async () => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        For EACH implementation type, provide a separate "outcome" (what the code would print/return) and "runtime" (e.g. "12ms").
        So each language should have: classCode, classOutcome, classRuntime, functionCode, functionOutcome, functionRuntime, recursiveCode, recursiveOutcome, recursiveRuntime.
        ...
        Return the result in JSON format matching this schema:
        {
          ...
          "code": {
            "python": { 
              "classCode": string, "classOutcome": string, "classRuntime": string,
              "functionCode": string, "functionOutcome": string, "functionRuntime": string,
              "recursiveCode": string, "recursiveOutcome": string, "recursiveRuntime": string
            },
            "cpp": { 
              "classCode": string, "classOutcome": string, "classRuntime": string,
              "functionCode": string, "functionOutcome": string, "functionRuntime": string,
                ...
                properties: {
                  python: {
                    ...
                    properties: {
                      ...
                      classOutcome: { type: Type.STRING },
                      classRuntime: { type: Type.STRING },
                      functionCode: { type: Type.STRING },
                      functionOutcome: { type: Type.STRING },
                      functionRuntime: { type: Type.STRING },
                      recursiveCode: { type: Type.STRING },
                      recursiveOutcome: { type: Type.STRING },
                      recursiveRuntime: { type: Type.STRING }
                    },
                    required: ["classCode", "functionCode", "recursiveCode"]
                  },
                  cpp: {
                    type: Type.OBJECT,
                    properties: {
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        notification.className = 'fixed top-6 right-6 z-[100] px-6 py-4 bg-emerald-600 text-white rounded-2xl shadow-2xl shadow-emerald-500/30 flex items-center gap-3 animate-slide-in';
        notification.innerHTML = `
          <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
          </svg>
          <span class="font-bold text-sm">Algorithm ${isEditing ? 'Updated' : 'Initialized'} Successfully!</span>
        `;
        document.body.appendChild(notification);
        
        // Auto-refresh data by triggering a custom event
        window.dispatchEvent(new CustomEvent('algorithms-updated'));
        
        // Redirect to /forge with reset state after delay
        setTimeout(() => {
          notification.remove();
        ...
        const notification = document.createElement('div');
        notification.className = 'fixed top-6 right-6 z-[100] px-6 py-4 bg-rose-600 text-white rounded-2xl shadow-2xl shadow-rose-500/30 flex items-center gap-3';
        notification.innerHTML = `
          <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
          </svg>
          <span class="font-bold text-sm">Failed to save algorithm</span>
        `;
        document.body.appendChild(notification);
      ...
      notification.innerHTML = `
        ...
        <span class="font-bold text-sm">Error saving algorithm</span>
      `;
      document.body.appendChild(notification);
            ...
            <Card variant="glass" className="p-6 sm:p-10">
              ...
              <div className="space-y-8">
                ...
                <div className="space-y-6">
                  {/* Class Implementation - Outcome & Runtime */}
                  {formData.code?.[activeLang]?.classCode && (
                    <div className="p-6 rounded-2xl bg-indigo-500/5 border border-indigo-500/10 space-y-4">
                      <div className="flex items-center gap-2 mb-4">
                        <Braces size={16} className="text-indigo-400" />
                        <h4 className="text-sm font-black text-white uppercase tracking-tight">Class Implementation - Output Settings</h4>
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-3">
                          ...
                          <textarea
                            value={formData.code?.[activeLang]?.classOutcome || ''}
                            onChange={e => updateCodeSnippet(activeLang, { classOutcome: e.target.value })}
                            placeholder="Expected output for class implementation..."
                            className="w-full bg-black border border-white/10 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-indigo-500 outline-none transition-all h-24 resize-none text-white placeholder:text-slate-700"
                          />
                        </div>
                        <div className="space-y-3">
                          <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">
                          ...
                          <Input
                            value={formData.code?.[activeLang]?.classRuntime || ''}
                            onChange={e => updateCodeSnippet(activeLang, { classRuntime: e.target.value })}
                            placeholder="e.g. 24ms"
                            icon={<Zap size={16} />}
                            className="py-3"
                          />
                        </div>
                  ...
                  {/* Function Implementation - Outcome & Runtime */}
                  {formData.code?.[activeLang]?.functionCode && (
                    <div className="p-6 rounded-2xl bg-emerald-500/5 border border-emerald-500/10 space-y-4">
                      <div className="flex items-center gap-2 mb-4">
                        <Cpu size={16} className="text-emerald-400" />
                        <h4 className="text-sm font-black text-white uppercase tracking-tight">Function Implementation - Output Settings</h4>
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-3">
                          <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">
                          ...
                          <textarea
                            value={formData.code?.[activeLang]?.functionOutcome || ''}
                            onChange={e => updateCodeSnippet(activeLang, { functionOutcome: e.target.value })}
                            placeholder="Expected output for function implementation..."
                            className="w-full bg-black border border-white/10 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-emerald-500 outline-none transition-all h-24 resize-none text-white placeholder:text-slate-700"
                          />
                        </div>
                        <div className="space-y-3">
                          <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">
                          ...
                          <Input
                            value={formData.code?.[activeLang]?.functionRuntime || ''}
                            onChange={e => updateCodeSnippet(activeLang, { functionRuntime: e.target.value })}
                  ...
                  {/* Recursive Implementation - Outcome & Runtime */}
                  {formData.code?.[activeLang]?.recursiveCode && (
                    <div className="p-6 rounded-2xl bg-amber-500/5 border border-amber-500/10 space-y-4">
                      <div className="flex items-center gap-2 mb-4">
                        <History size={16} className="text-amber-400" />
                        <h4 className="text-sm font-black text-white uppercase tracking-tight">Recursive Implementation - Output Settings</h4>
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-3">
                          <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">
                          ...
                          <textarea
                            value={formData.code?.[activeLang]?.recursiveOutcome || ''}
                            onChange={e => updateCodeSnippet(activeLang, { recursiveOutcome: e.target.value })}
                            placeholder="Expected output for recursive implementation..."
                            className="w-full bg-black border border-white/10 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-amber-500 outline-none transition-all h-24 resize-none text-white placeholder:text-slate-700"
                          />
                        </div>
                        <div className="space-y-3">
                          <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">
                          ...
                          <Input
                            value={formData.code?.[activeLang]?.recursiveRuntime || ''}
                            onChange={e => updateCodeSnippet(activeLang, { recursiveRuntime: e.target.value })}
                            placeholder="e.g. 18ms"
                            icon={<Zap size={16} />}
                            className="py-3"
                          />
                        </div>
