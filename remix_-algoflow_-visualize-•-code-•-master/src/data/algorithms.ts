...
export const algorithms: Algorithm[] = [
  {
    ...
    for (int i = 0; i < n-1; i++) {
      ...
      rust: {
        functionCode: `fn bubble_sort(arr: &mut [i32]) {
    let n = arr.len();
    for i in 0..n {
        for j in 0..n - i - 1 {
            if arr[j] > arr[j + 1] {
    ...
    arr[j], arr[j + 1] = arr[j + 1], arr[j]`,
          language: 'python',
          explanation: 'The core of bubble sort is comparing adjacent elements and swapping them if they are in the wrong order.'
      },
      {
          id: 'q2',
          ...
          language: 'cpp',
          explanation: 'In a recursive implementation, if the array size is 1, it is already sorted.'
      }
    ]
  },
  {
    ...
    if high >= low:
      ...
      cpp: {
        functionCode: `int binarySearch(int arr[], int l, int r, int x) {
    while (l <= r) {
        int m = l + (r - l) / 2;
        if (arr[m] == x) return m;
        if (arr[m] < x) l = m + 1;
    ...
    questions: [
      {
        id: 'bs-q1',
        text: 'How do you calculate the middle index in Rust to avoid overflow?',
        answer: `let mid = low + (high - low) / 2;`,
        language: 'rust',
        explanation: 'Using (low + high) / 2 can cause integer overflow if low + high exceeds the maximum value of the integer type.'
      },
      {
        id: 'bs-q2',
        text: 'Implement the condition to search the right half in C.',
        answer: `if (arr[mid] < target) low = mid + 1;`,
        language: 'c',
        explanation: 'If the middle element is less than the target, the target must be in the right half of the sorted array.'
      }
    ]
  },
  {
    ...
    return merge(left, right)`,
      ...
      cpp: {
        functionCode: `void mergeSort(int arr[], int l, int r) {
    if (l < r) {
        int m = l + (r - l) / 2;
        mergeSort(arr, l, m);
        mergeSort(arr, m + 1, r);
    ...
    if (l < r) {
      ...
      rust: {
        functionCode: `fn merge_sort(arr: &mut [i32]) {
    let mid = arr.len() / 2;
    if mid == 0 { return; }
  ...
  },
  {
    ...
    for (auto i = adj[v].begin(); i != adj[v].end(); ++i)
      ...
      c: {
        functionCode: `void DFS(struct Graph* graph, int vertex) {
    ...
    while (temp != NULL) {
      ...
      rust: {
        functionCode: `fn dfs(graph: &Graph, start: NodeId) {
  ...
  },
  {
    ...
    code: {
      python: {
        functionCode: `from collections import deque
def bfs(graph, start):
    ...
    while(!q.empty()) {
      ...
      c: {
        functionCode: `void bfs(struct Graph* graph, int startNode) {
    ...
    while (!isEmpty(q)) {
      ...
      rust: {
        functionCode: `fn bfs(graph: &Graph, start: NodeId) {
    let mut visited = HashSet::new();
...
def dijkstra(graph, start):
    ...
    return distances`,
      ...
      cpp: {
        functionCode: `void dijkstra(int s) {
    ...
    while (!pq.empty()) {
      ...
      c: {
        functionCode: `void dijkstra(int graph[V][V], int src) {
    ...
    for (int count = 0; count < V - 1; count++) {
      ...
      rust: {
        functionCode: `fn dijkstra(graph: &Graph, start: NodeId) -> HashMap<NodeId, i32> {
