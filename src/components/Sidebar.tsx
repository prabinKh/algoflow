...
export default function Sidebar({ 
  ...
  return (
    <aside className={cn(
      "fixed top-0 left-0 h-screen bg-[#0d0d0d] border-r border-slate-800 z-40 transition-all duration-300 overflow-hidden flex flex-col",
      ...
      <div className="p-6 flex items-center gap-3 border-b border-slate-800">
        ...
        <div>
          <h2 className="font-black text-xl tracking-tight leading-none text-white">AlgoFlow</h2>
      ...
      <div className="p-4">
        <button 
          ...
          className="w-full flex items-center gap-3 p-3 bg-indigo-900/20 text-indigo-400 rounded-xl border border-indigo-900/30 hover:bg-indigo-900/40 transition-all mb-2"
        >
          <BookOpen size={18} />
        ...
        {isAdmin && (
          <button 
            ...
            className="w-full flex items-center gap-3 p-3 hover:bg-slate-800 text-slate-400 rounded-xl transition-all mb-4"
          >
        ...
        <div className="relative group">
          ...
          <input 
            ...
            className="w-full bg-slate-900 border-none rounded-xl py-2.5 pl-10 pr-4 text-sm focus:ring-2 focus:ring-indigo-500 outline-none transition-all text-white placeholder:text-slate-500"
          />
        </div>
        {isAdmin && (
          <button 
            ...
            <div key={category.name} className="space-y-1">
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="p-1.5 bg-slate-800 rounded-md text-slate-500 group-hover:text-indigo-500 transition-colors">
                    <Icon size={16} />
                  </div>
                  <span className="text-sm font-bold text-slate-400">{category.name}</span>
                </div>
                  ...
                  >
                    {algosInCategory.map((algo) => (
                      ...
                      >
                        ...
                        {selectedAlgoId !== algo.id && (
                          <div className="absolute right-2 opacity-0 group-hover:opacity-100 transition-opacity text-slate-500">
